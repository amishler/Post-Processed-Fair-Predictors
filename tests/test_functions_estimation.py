...
def test_est_cFPR_and_cFNR_overall(mock_data):
    ...
    fpr = est_cFPR_overall(mock_data, R='R', outcome='phihat')
    fnr = est_cFNR_overall(mock_data, R='R', outcome='phihat')
    assert 0 <= fpr <= 1
    assert 0 <= fnr <= 1
...
def test_fair_derived(mock_data):
    X = ['X1', 'X2']
    model = LogisticRegression()
    result = fair_derived(data=mock_data, A='A', X=X, R='R', D='D', Y='Y',
                             learner_pi=model, learner_mu=model,
