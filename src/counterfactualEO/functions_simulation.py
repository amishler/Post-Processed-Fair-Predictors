...
def _eval_one_theta(n_val, mc_iter, theta_row, epsilon_pos, epsilon_neg, 
                    data_val, ci=0.95, ci_scale='logit'):
    """
...
def simulate_task1_metrics_to_df(res, n_arr, setting, data_val, ci=None, 
    ...
    Args:
        ...
        ci (float): Confidence interval level, or None for no confidence
            intervals. Set to None by default, because the point of task 1 is to
            evaluate the variability in theta-hat itself and then estimate the
            metrics as accurately as possible for those theta-hat values on a
            large simulated validation dataset where the underlying
            counterfactual outcomes are known. Interest is in estimation error
            for theta-hat, not in estimation of the metric values.
