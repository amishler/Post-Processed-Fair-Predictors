...
def _eval_one_theta(n_val, mc_iter, theta_row, epsilon_pos, epsilon_neg, data_val, ci, ci_scale='logit'):
    """
    Evaluate a single theta and return its metrics as a DataFrame row.
    """
    df = metrics_post(theta_row, data_val, A='A', R='R', outcome='mu0', ci=ci, ci_scale=ci_scale)
...
def simulate_task1_metrics_to_df(res, n_arr, setting, data_val, ci=0.95, ci_scale='logit', n_jobs=-1):
    """
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(_eval_one_theta)(n_val, mc_iter, theta_row, epsilon_pos, epsilon_neg, data_val, ci, ci_scale)
