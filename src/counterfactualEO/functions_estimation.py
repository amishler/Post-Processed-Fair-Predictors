...
def train_nuisance(train, test, A, X, R, D, Y, learner_pi, learner_mu,
    ...
    Note that this only handles numeric features. Categorical features need to
    ...
    """
    # TODO: allow for separate models for the two levels of the sensitive feature A
...
def risk_coef(data, a, r, A='A', R='R', outcome='phihat', cost_fpr=1, cost_fnr=1):
    ...
    $\theta_{a, r}$ in a weighted misclassification loss function, where 
    false positive and false negative errors can be assigned different cost.
    The scale of the weights is arbitrary, but the ratio between them is
    important.  The default is to assign equal weight to both types of error.
    ...
    """
    out = (((data[A] == a) & (data[R] == r)) * (cost_fpr - (cost_fpr + cost_fnr) * data[outcome])).mean()

    return out


def risk_coefs(data, A='A', R='R', outcome='phihat', cost_fpr=1, cost_fnr=1):
    """Compute the risk coefficients $\beta_{a, r}$ for all a, r.

    This function returns the four coefficients corresponding to all values of 
    (A, R) ∈ {0,1} × {0,1}, using the specified outcome and optional weights 
    for false positives and false negatives.

    These coefficients are used in the weighted misclassification objective:
    loss = sum_{a,r} β_{a,r} * θ_{a,r}. See `risk_coef` for more details.
    ...
    coefs = [risk_coef(data, a, r, A, R, outcome, cost_fpr, cost_fnr) for a, r in ar_list]
...
def fair_derived_split(data, A, X, R, D, Y, learner_pi, learner_mu, epsilon_pos,
    ...
    obj = risk_coefs(data_opt, A=A, R=R, outcome=outcome, cost_fpr=cost_fpr, cost_fnr=cost_fnr)
...
def fair_derived_crossfit(data, A, X, R, D, Y, learner_pi, learner_mu,
                          ...
                          trunc_pi=0.975, cost_fpr=1, cost_fnr=1, random_state=None):
    ...
    if estimator_type == 'pooled':
        ...
        obj = risk_coefs(pooled_data, A=A, R=R, outcome=outcome, cost_fpr=cost_fpr, cost_fnr=cost_fnr)
        ...
        for train_idx, test_idx in kf.split(data):
            ...
            obj = risk_coefs(data_test, A=A, R=R, outcome=outcome, cost_fpr=cost_fpr, cost_fnr=cost_fnr)
